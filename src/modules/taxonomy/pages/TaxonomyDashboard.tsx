...
import { TAXONOMY_TYPES_REGISTRY } from "../registry";
import { TAXONOMY_ICONS_MAP } from "../constants";
      ...
      <main className="flex-1 container mx-auto px-4 py-12 max-w-6xl space-y-8">
        ...
        <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold bg-primary/10 text-primary px-2.5 py-1 rounded-full w-fit mb-2">
              ...
              Taxonomy Engine Active
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight text-foreground">
              Axevora Admin Taxonomy Panel
            </h1>
            <p className="text-muted-foreground text-sm mt-1">
              Unified schema-based taxonomy classifications dashboard.
            </p>
          </div>
        </section>
        ...
        <Card className="border-primary/20 bg-gradient-to-r from-primary/5 to-purple-500/5">
          <CardContent className="flex items-center gap-4 py-6">
            ...
            <div>
              <h4 className="font-bold text-sm text-foreground">Universal Classification Architecture</h4>
              <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed">
                Categories, Tags, Brands, Collections, Labels, Stores, and Attributes are dynamically registered under the taxonomy engine database registry. This decoupled model enables other modules (like Deals and CMS) to use shared taxonomies.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <section className="space-y-4">
          <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
            ...
            Registered Taxonomy Classifications
          </h3>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {TAXONOMY_TYPES_REGISTRY.map((t) => {
              const IconComponent = TAXONOMY_ICONS_MAP[t.iconName || "Folder"] || TAXONOMY_ICONS_MAP.Folder;
              return (
                <Card key={t.type} className="hover:border-primary/20 hover:shadow-md transition-all group">
                  <CardHeader className="pb-4">
                    ...
                    <CardDescription>
                      Hierarchy: {t.supportHierarchy ? "Hierarchical (Recursive)" : "Flat structure"}
                    </CardDescription>
                  </CardHeader>
